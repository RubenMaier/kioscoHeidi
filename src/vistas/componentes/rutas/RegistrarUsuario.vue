...
export default {
  ...
  methods: {
    ...
    registrarUsuario() {
      ...
      this.axios
        .get("/usuarios/obtener/username/" + this.usuario.username)
        .then(res => {
          if (this.usuario.username === res.data.username)
      ...
      this.axios
        .get("/usuarios/obtener/email/" + this.usuario.email)
        .then(res => {
          if (this.usuario.email === res.data.email)
            this.resultado.username = "El email ya existe en la base de datos";
        });
      if (!this.usuario.nombre) this.resultado.nombre = "Ingrese un nombre";
      if (!this.usuario.apellido)
        this.resultado.apellido = "Ingrese un apellido";
      if (!this.usuario.rol) this.resultado.rol = "Ingrese un rol";

      if (
        !this.resultado.username &&
        !this.resultado.passowrd &&
        ...
        this.axios
          .post("/usuarios/registrar", this.usuario)
